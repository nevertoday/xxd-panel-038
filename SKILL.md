---
name: xxd-panel-038
description: "Create XXD Panel 038 artwork from supplied photos in four combinable modes: photo above/warm source-aware patchwork textile collage with active whitespace and stitched copy below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Use for the exact 038 style; reject children's craft projects, literal photo tracing, evenly tiled quilts, fixed beige or brown palettes, fake vintage stains, cute stickers, printed fabric simulation without fibre, meaningless decoration, template collage."
---

# XXD Panel 038 · 人文主义拼布贴画

Understand the photograph's unspoken relation and rebuild only its essential forms as a warm contemporary patchwork collage made from checks, linen, washed burlap, cut cloth, frayed edges, patches, and visible hand stitching.
...
Reject the result as generic if an unrelated photograph could replace the source without materially changing recognition, construction, placement, material, colour relation, whitespace, or copy. The operative exclusions are: children's craft projects, literal photo tracing, evenly tiled quilts, fixed beige or brown palettes, fake vintage stains, cute stickers, printed fabric simulation without fibre, meaningless decoration, template collage.
...
Distil one short phrase from the source's hidden emotion, action, relation, or metaphor rather than merely naming the object or place. Render it as fine native embroidery, hand stitching, or restrained handwriting that continues, pauses, or hides within seams and negative space.
...
Use only current attachments, explicit paths, or a previously supplied image clearly identified by the user. Never scan Desktop, `~/Desktop/xxd/xxd-panel-038/`, or historical task folders for a substitute.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，038 设计在下）
   2. 左右对照（原图在左，038 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-038-prompt.en.md` or `references/xxd-panel-038-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 038 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 038 transformation on the right.
   - `design-only`: generate the 038 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 038 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 038-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 038-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
`scripts/compose_panel.py` remains available for deterministic recovery and audit. Trigger it only under step 11. When used, generate a same-aspect design asset from the full 038 prompt, preserve the source without destructive crop or stretch, and document which fallback condition applied. A direct complete-canvas success must not be split and recomposed again.
...
```text
~/Desktop/xxd/xxd-panel-038/<fresh-task>/
└── source-01/
...
Preserve explicit subject, mode set, output count, exact pixels or ratios, wallpaper relationship, copy mode, exact wording, locale, and intended meaning. User overrides may alter these variables but do not silently authorise abandoning source identity, the 038 aesthetic motive, fresh-task isolation, raster-only output, privacy, or verification. If the user explicitly requests a different aesthetic, acknowledge that it leaves this Skill's style rather than pretending it remains 038.

## References

- Read `references/xxd-panel-038-prompt.en.md` or `references/xxd-panel-038-prompt.zh-CN.md` immediately before generation.
- `references/038-source.md` archives the original style brief and is evidence, not an implicit 3:4 default.
