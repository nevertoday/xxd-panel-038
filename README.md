...
原始审美约束与拒绝项只存在于[原始提示词](references/038-source.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-038-prompt.en.md)

## 原始提示词优先，而不是二次导演

`references/038-source.md` 是本项目唯一的创作与审美权威。Skill 不再额外总结或扩写它，也不会统一规划颜色、色板、美学动机、标题或微文案。原始提示词要求怎样处理颜色、材料、构图、留白与文字，GPT Image 2 就按那套逻辑执行。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/038-source.md) · [英文运行适配器](references/xxd-panel-038-prompt.en.md) · [中文运行适配器](references/xxd-panel-038-prompt.zh-CN.md)
