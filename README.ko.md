...
전체 미적 제약과 금지 항목은 Skill과 생성 프롬프트에 있습니다. 원문 미학을 보존하지만 역사적인 3:4 화면을 숨은 기본값으로 만들지 않습니다. [SKILL.md](SKILL.md) · [production prompt](references/xxd-panel-038-prompt.en.md)
...
| 모드 | 화면비 규칙 | 결과물 |
| --- | --- | --- |
| `top-bottom` | 사용자가 확인한 전체 캔버스 | 완성 캔버스를 한 번에 생성: 위에 고충실도 원본, 아래에 038 디자인, 약 50/50 |
| `left-right` | 사용자가 확인한 전체 캔버스 | 완성 캔버스를 한 번에 생성: 왼쪽에 고충실도 원본, 오른쪽에 038 디자인, 약 50/50 |
| `design-only` | 사용자가 확인한 전체 캔버스 | 038 디자인이 전체를 채우며 원본 사진은 보이지 않음 |
...
이 프로젝트의 문구 규칙: 원본의 주체, 동작, 관계, 감정 또는 함의에 강하게 결속된 짧은 제목 하나만 만들고 필요한 미세 주석을 최소화합니다. 대상 언어 고유의 문자 구조로 재료, 윤곽, 축과 여백에 통합하며 완성 문구는 그대로 유지합니다.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/038-source.md) · [영문 생성 프롬프트](references/xxd-panel-038-prompt.en.md) · [중문 생성 프롬프트](references/xxd-panel-038-prompt.zh-CN.md)
